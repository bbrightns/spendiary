    ...
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-line bg-surface px-4 py-5 lg:flex flex-col justify-between select-none">
      <div className="flex flex-col gap-6">
        ...
        <nav className="flex flex-col gap-1">
          <span className="px-3 text-[11px] font-bold uppercase tracking-wider text-ink-faint mb-1">
            Menu
          </span>
          {navItems.map((item) => {
            const isActive =
      ...
      <div className="flex flex-col gap-2 pt-4 border-t border-line">
        ...
        <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-surface-muted/50 border border-line/40">
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="grid h-8 w-8 place-items-center rounded-lg text-ink-muted hover:bg-surface hover:text-ink hover:shadow-xs transition-all cursor-pointer"
            title="Toggle theme"
