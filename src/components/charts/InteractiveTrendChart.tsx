...
export interface InteractiveTrendChartProps {
  title?: string
...
export function InteractiveTrendChart({
  ...
  if (history && history.length === 1) {
    ...
    const [y0, mo0, d0] = single.date.split('-').map(Number)
    ...
    const colorVar = accentColor ?? 'var(--color-cash)'
      ...
      <div className="relative flex flex-col justify-between h-full">
        ...
        <div className="relative px-4 pt-2 pb-3">
          <svg viewBox="0 0 800 200" className="w-full" style={{ height: '150px', display: 'block' }} aria-hidden>
            ...
            <path d="M16,100 L784,100 L784,200 L16,200Z" fill={`url(#${gradientId})`} />
  ...
  const baseIsUp = change >= 0
  const colorVar = baseIsUp ? (accentColor ?? 'var(--color-cash)') : 'var(--color-loss)'
  ...
  const isUp = activeChangeFromStart >= 0

  let activeDateLabel = ''
  if (activePoint?.date) {
    const [ay, amo, ad] = activePoint.date.split('-').map(Number)
    activeDateLabel = !isNaN(ay)
      ? new Date(ay, amo - 1, ad).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
      : activePoint.date
  }

  const findNearestIndex = (clientX: number, rect: DOMRect) => {
    const normalizedX = ((clientX - rect.left) / rect.width) * W
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    setHoveredIndex(findNearestIndex(e.clientX, rect))
  }

  const handleTouchMove = (e: React.TouchEvent<SVGSVGElement>) => {
    if (!e.touches[0]) return
    const rect = e.currentTarget.getBoundingClientRect()
    setHoveredIndex(findNearestIndex(e.touches[0].clientX, rect))
  }
    ...
    <div className="relative flex flex-col justify-between h-full">
      ...
      <div className="relative px-4 pt-2 pb-3">
        <svg
          ...
          role="img"
          aria-label={`${title} chart from ${startDate} to ${endDate}. Current value is ${thb(activePoint?.value ?? last.value)}`}
        >
...
export function InteractivePortfolioChart({ history }: { history: NetWorthSnapshot[] }) {
  return (
    <InteractiveTrendChart
      title="Portfolio Value Trend"
      history={history}
      accentColor="var(--color-funds)"
      gradientId="pfInteractiveGrad"
    />
  )
}
